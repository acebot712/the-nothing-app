...
const getUserById = async (userId) => {
  try {
    ...
    logger.error(
      `Unexpected error retrieving user by ID ${userId}: ${error.message}`,
    );
...
const createUser = async (userData) => {
  try {
    ...
    if (existingError) {
      logger.error(
        `Error checking for existing user: ${existingError.message}`,
      );
...
const updateUserTier = async (userId, tierInfo) => {
  try {
    ...
    if (error instanceof ApiError) throw error;
    logger.error(
      `Unexpected error updating user tier ${userId}: ${error.message}`,
    );
...
const updateLeaderboardForUser = async (userId, userData) => {
  try {
    ...
    if (checkError && checkError.code !== "PGRST116") {
      ...
      logger.error(
        `Error checking leaderboard for user ${userId}: ${checkError.message}`,
      );
      throw new ApiError(
        `Error updating leaderboard: ${checkError.message}`,
        500,
      );
    }
    ...
    if (existingEntry) {
      ...
      if (error) {
        logger.error(
          `Error updating leaderboard for user ${userId}: ${error.message}`,
        );
      ...
      if (error) {
        logger.error(
          `Error creating leaderboard entry for user ${userId}: ${error.message}`,
        );
        throw new ApiError(
          `Error creating leaderboard entry: ${error.message}`,
          500,
        );
      }
    ...
    logger.error(
      `Error with leaderboard update for user ${userId}: ${error.message}`,
    );
