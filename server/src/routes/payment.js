...
const { createPaymentIntent, verifyPaymentIntent, handleWebhookEvent } = require('../utils/stripe');
...
router.post('/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  try {
    ...
    if (event.success && event.event === 'payment_intent.succeeded' && event.paymentIntentId) {
      try {
        ...
        logger.info(`Webhook: Processing successful payment ${event.paymentIntentId}`);
        
        // In a real implementation, you'd verify the payment and update the user
        // For now, we'll log it
        logger.info(`Webhook: Would update user for payment ${event.paymentIntentId}`);
      } catch (processingError) {
        logger.error(`Error processing webhook payment: ${processingError.message}`);
      }
    }
  } catch (error) {
